...
c	
      ...
      SUBROUTINE LmDifEzCopy(FCN,M,N,X,factor,FVEC,TOL,
	1								iErrCode,stdErr)
C     **********
C
...
C     **********
	implicit doubleprecision (A-H,O-Z)
      parameter(MAXDAT=333,MAXPARMS=11,LWA=5*MAXPARMS+MAXDAT) 
...
c     *                 IPVT,QTF,WA1,WA2,WA3,WA4)
      CALL LMDIFCopy(FCN,M,N,X,FVEC,FTOL,XTOL,GTOL,MAXFEV,EPSFCN,
...
c			do mPt=1,nPts
c				covar(iParm,jParm)=covar(iParm,jParm)
...
C 12480 BYTES REQUIRED FOR MEMBER LMDif1
      ...
      SUBROUTINE MATINVCopy(A,NROWA,N,IPVT,WORK,INFO)
	!In-place matrix inverse.  A becomes inverse of A on return.
      ...
      CALL DGEDIcopy(A,NROWA,N,IPVT,DET,WORK,01)
      RETURN
      END
      SUBROUTINE DGEDIcopy(A,LDA,N,IPVT,DET,WORK,JOB)
      INTEGER LDA,N,IPVT(1),JOB
      DOUBLE PRECISION A(LDA,1),DET(2),WORK(1)
C
C     DGEDIcopy COMPUTES THE DETERMINANT AND INVERSE OF A MATRIX
...
C
  ...
  150 CONTINUE
      RETURN
      END
C  10240 BYTES REQUIRED FOR MEMBER DGEDIcopy
      SUBROUTINE DAXPYCopy(N,DA,DX,INCX,DY,INCY)
C
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      SUBROUTINE  DSCALCopy(N,DA,DX,INCX)
C
...
C  3280 BYTES REQUIRED FOR MEMBER DSCAL
      SUBROUTINE  DSWAPCopy (N,DX,INCX,DY,INCY)
C
...
C  4400 BYTES REQUIRED FOR MEMBER DSWAP
      SUBROUTINE DGEFACopy(A,LDA,N,IPVT,INFO)
...
C                     INDICATE THAT DGESL OR DGEDIcopy WILL DIVIDE BY ZERO
...
C     SUBROUTINES AND FUNCTIONS
C
C     BLAS DAXPY,DSCAL,IDAMAXcopy
C
C     INTERNAL VARIABLES
C
      DOUBLE PRECISION T
      INTEGER IDAMAXcopy,J,K,KP1,L,NM1
C
C
...
C
         L = IDAMAXcopy(N-K+1,A(K,K),1) + K - 1
         IPVT(K) = L
C
...
C  8240 BYTES REQUIRED FOR MEMBER DGEFA
      INTEGER FUNCTION IDAMAXcopy(N,DX,INCX)
C
...
C
   ...
   30 CONTINUE
      RETURN
      END
C  2960 BYTES REQUIRED FOR MEMBER IDAMAXcopy
...
C
      SUBROUTINE LMDIFCopy(FCN,M,N,X,FVEC,FTOL,XTOL,GTOL,MAXFEV,EPSFCN,
...
C
         IFLAG = 2
         CALL FDJAC2Copy(FCN,M,N,X,FVEC,FJAC,LDFJAC,IFLAG,EPSFCN,WA4)
         NFEV = NFEV + N
         IF (IFLAG .LT. 0) GO TO 300
C
...
C
      END
      SUBROUTINE LMPARCopy(N,R,LDR,IPVT,DIAG,QTB,DELTA,PAR,X,SDIAG,WA1,
     *                 WA2)
      INTEGER N,LDR
      INTEGER IPVT(N)
...
C
      END
      SUBROUTINE FDJAC2Copy(FCN,M,N,X,FVEC,FJAC,LDFJAC,IFLAG,EPSFCN,WA)
