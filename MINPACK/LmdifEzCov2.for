...
c	
      ...
      SUBROUTINE LmDifEz(FCN,M,N,X,factor,FVEC,TOL,iErrCode,stdErr)
C     **********
C
...
C         External LmDevFcn
C         ...
C		Call LmDifEz(LmDevFcn,nPts,nParms,parm,factor,deviate,tol,iErrCode,stdErr)
...
C
C     **********
	USE GlobConst, only:errMsgPass
      implicit doubleprecision (A-H,O-Z)
      parameter(MAXDAT=4444,MAXPARMS=211,LWA=5*MAXPARMS+MAXDAT) 
...
c      DATA FACTOR,ZERO /.1,0.0D0/
      DATA ZERO /0.0D0/
        ...
        errMsg(4)='FVEC IS ORTHOGONAL TO THE COLUMNS OF THE JACOBIAN.'
        errMsg(5)='nCalls TO FCN WITH IFLAG = 1 HAS REACHED 100*(N+1).'
        errMsg(6)='TOL TOO SMALL. NO FURTHER REDUCTION IN SSQ POSSIBLE.'
        errMsg(7)='TOL TOO SMALL. NO FURTHER IMPROVEMENT POSSIBLE.'
      INFO = 0
	LDFJAC=MAXDAT
C
