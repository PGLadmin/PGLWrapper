MODULE DLLConst
	...
	data EosName/'PR','ESD96','PRWS','ESD-MEM2','SPEADMD','Flory-MEM2','NRTL','SpeadGamma-MEM2','SPEAD11','PcSaft(Gross)', &
	  'tcPRq','GCESD','GCESD(Tb)','TransSPEAD','GcPcSaft','GcPcSaft(Tb)','tcPR-GE(W)','ESD2','LsgMem2','SptPcSaft'/
...
integer function SETSTRING(tag, value)
	USE GlobConst
    character*255 tag, value, errMsg !, local
...
integer function InitPGLDLL(errMsg)
	...
	close(51)
    ! Finished with ReadOptions from file.
    if (LOUD)then
        dumpUnit=686
