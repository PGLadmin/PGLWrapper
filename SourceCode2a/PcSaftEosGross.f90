...
subroutine MATINVg ( N, M, AA, BB, DET )	 !JRE added the g for Gross. This file is just for PcSaftGross.
  USE GlobConst, only: LOUD					 !JRE added LOUD indicator to supress console writing if LOUD==.FALSE.
  use PARAMETERS, only: dp
  implicit none
  !-----------------------------------------------------------------------------
  integer, intent(in)                        :: N
  integer, intent(in)                        :: M
  real(dp), dimension(N,N), intent(in out)   :: AA
  real(dp), dimension(N,M), intent(in out)   :: BB
  real(dp), intent(out)                      :: DET

  !-----------------------------------------------------------------------------
  integer                                    :: i, j, k, ik, jk
  real(dp), PARAMETER                        :: EPSMACH = 2.E-16
  real(dp)                                   :: PV, PAV, TT    
...
subroutine jacobi_eigenvalue ( n, a, it_max, v, d, it_num, rot_num )
  ...
  !-----------------------------------------------------------------------------
  integer, intent(in)                        :: n
  real(dp), dimension(n,n), intent(in out)   :: a
  integer, intent(in)                        :: it_max
  real(dp), dimension(n,n), intent(out)      :: v
  real(dp), dimension(n), intent(out)        :: d
  ...
  real(dp)                                   :: theta
  real(dp)                                   :: thresh
  real(dp), dimension(n)                     :: bw
  real(dp), dimension(n)                     :: w, zw
  !-----------------------------------------------------------------------------
  ...
  do while ( it_num < it_max )
     ...
     do p = 1, n
        do q = p + 1, n
           ...
           !--------------------------------------------------------------------
           if ( 4 < it_num .and. abs( termp - abs(d(p)) ) < machine_eps  &
                .and. abs( termq -abs(d(q)) ) < machine_eps ) then
              ...
              if ( abs( term - abs( h ) ) < machine_eps ) then
                 t = a(p,q) / h
              else
                 theta = 0.5 * h / a(p,q)
...
!!$  real(dp), parameter                        :: abserr = 1.E-08
!!$  !-----------------------------------------------------------------------------
