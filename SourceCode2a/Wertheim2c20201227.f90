...
!	enddo
	...
	!Also, compute FAsolv, FDsolv, and rmsErrSolv
	ssqErr=0
	...
	DO iComp=1,nComps
		do iType=1,nTypes(iComp)
			...
			deltaAlphaA(iComp,iType)=0
			deltaAlphaD(iComp,iType)=0
			DO jComp=1,nComps
            ...
            if(deltaAlphaA(iComp,iType) < 0)then
                if(  ABS( deltaAlphaA(iComp,iType) ) < 1.D-6  )deltaAlphaA(iComp,iType)=ABS(deltaAlphaA(iComp,iType))  !roundoff?
                if(     ( deltaAlphaA(iComp,iType) ) < 0 .and. LOUD  )pause 'deltaAlphaA < 0???'  !roundoff?
            endif
            if(deltaAlphaD(iComp,iType) < 0)then
                if(ABS( deltaAlphaD(iComp,iType) ) < 1.D-6)deltaAlphaD(iComp,iType)=ABS(deltaAlphaD(iComp,iType))  !roundoff?
                if(   ( deltaAlphaD(iComp,iType) ) < 0 .and. LOUD  )pause 'deltaAlphaD < 0???'  !roundoff?
            endif
	...
	if(iDoSolvation==1)then  !checkup: 1E-111  ! Refine from ESD96 if necessary.
		write(*,'(a,3F10.6,3F10.1)')' x1,XA(1,2),XA(1,3),XA(2,2):',xFrac(1),XA(1,2),XA(1,3),XA(2,2) 
		write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) ,deltaAlphaA(1,3),deltaAlphaD(2,2) 
		if(LOUD)pause 'Wertheim: 96 solution'
		picard=0.1D0
		do iComp=1,nComps
			do iType=1,nTypes(iComp)
		...
		enddo
		write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) ,deltaAlphaA(1,3),deltaAlphaD(2,2) 
		if(LOUD)pause 'Wertheim: Asymptotic estimate. Calling XsolJre'
		call XsolJre1a(xFrac,rho,tKelvin,rdfContact,nComps,ierXsol)	 ! XA,XD,XC passed through module Assoc.
		write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) ,deltaAlphaA(1,3),deltaAlphaD(2,2) 
		if(LOUD)pause 'Wertheim: After Calling XsolJre'
				...
				DO jComp=1,nComps
					do jType=1,nTypes(jComp)
						...
						IF (iEosOpt.EQ.5.or.iEosOpt.EQ.8.or.iEosOpt.eq.9) THEN
							dHadij=( eAcceptorKcal_mol(iComp,iType)+eDonorKcal_mol(jComp,jType) )/2*(1-aBipAD(indexi,indexj))	!UpdatedbyEM
							dHdaij=( eDonorKcal_mol(iComp,iType)+eAcceptorKcal_mol(jComp,jType) )/2*(1-aBipAD(indexi,indexj)) !UpdatedbyEM
						ELSEIF (iEosOpt.EQ.4) THEN
							dHadij=( eHbKcal_mol(iComp,iType)+eHbKcal_mol(jComp,jType) )/2*(1-HIJ(iComp,jComp))
						ENDIF
						yHBadij=EXP(dHadij/tKelvin/1.987D-3)-1
						yHBdaij=EXP(dHdaij/tKelvin/1.987D-3)-1
						beps_Yhb=1  !anticipate divide by zero problem if Yhb=0
						if(yHBadij > 1e-4) beps_Yhb=dHadij/1.987D-3/(tKelvin*yHBadij)
						call AlphaSp(iComp,iType,jComp,jType,tKelvin,rho,rdfContact,bVolMix,alphADij,alphDAij,alphCCij)
						dAlphaAD= alphADij*beps_Yhb*(yHBadij+1)  !dAlphAD
						beps_Yhb=1  !anticipate divide by zero problem if Yhb=0
						if(yHBdaij > 1e-4) beps_Yhb=dHdaij/1.987D-3/(tKelvin*yHBdaij)
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC      
	...
	iter=0
	itMax=11
	do while(rmsErr > 1E-10.and.iter < itMax)
		iter=iter+1
		rmsErr=0
		DO iComp=1,nComps
			do iType=1,nTypes(iComp)
		...
		rmsOld=rmsErr
		write(*,'(a,6F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) !,deltaAlphaA(1,3) 
	enddo ! while(rmsErr > tol)
	if(iter > itMax)then
		ierCode=2
		if(LOUD)pause 'Error in XsolJre: iter>itMax'
	else
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC      
	...
	!DIMENSION xaOld(NMX,maxTypes),xdOld(NMX,maxTypes),faOld(NMX,maxTypes),fdOld(NMX,maxTypes)
	DoublePrecision sumDoni(maxTypes),sumDonj(maxTypes),sumAcci(maxTypes),sumAccj(maxTypes)
	WegFreq(iter)=ABS(MOD(FLOAT(iter),FLOAT(iWegFreq)))	! WegFreq=0 if iter is multiple of iWegFreq
	sumDoni = 0
	sumAcci = 0
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	IF (iEosOpt==5 .or. iEosOpt==8 .or. iEosOpt==9) THEN
		dHadij=( eAcceptorKcal_Mol(iComp,iType)+eDonorKcal_Mol(jComp,jType) )/2*( 1-aBipAD(indexi,indexj) )
		dHdaij=( eDonorKcal_Mol(iComp,iType)+eAcceptorKcal_Mol(jComp,jType) )/2*( 1-aBipDA(indexi,indexj) )
	ELSE
		dHadij=( eHbKcal_Mol(iComp,iType)+eHbKcal_Mol(jComp,jType) )/2*( 1-HIJ(indexi,indexj) )
		dHdaij=( eHbKcal_Mol(iComp,iType)+eHbKcal_Mol(jComp,jType) )/2*( 1-HIJ(indexi,indexj) )
	ENDIF
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
