...
111			continue  !transfer from read(40,...,END=111)dumString
			...
			IF(idBase.EQ.idComp(iComp))THEN
				...
				if(LOUDER)write(*,'(a,i3,9(i3,i5))')' nTypes,Nd,Id',nTypes(iComp),(nFg(iComp,iType),idType(iComp,iType),iType=1,nTypes(iComp))
				if(LOUDER)write(*,'(a,i3,<nTypes(iComp)>i5)')' nTypes,#Fgi',nTypes(iComp),(nFg(iComp,iType),iType=1,nTypes(iComp))
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	DO iComp=1,nComps	!Note: assume all types hbond. It just means summing over some zeroes.
        ...
        if(LOUDER)pause 'Check HB parms.'
	enddo !iComp
	CLOSE(inHbFile)
