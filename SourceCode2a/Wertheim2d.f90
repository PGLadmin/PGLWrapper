...
!	enddo
	...
	if(iDoSolvation==1)then  !checkup: 1E-111  ! Refine from ESD96 if necessary.
		if(LOUD.and.initCall) write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) ,deltaAlphaA(1,3),deltaAlphaD(2,2) 
		if(LOUD.and.initCall)pause 'Wertheim: 96 solution'
		do iComp=1,nComps
			do iType=1,nTypes(iComp)
		...
		enddo
		if(LOUD.and.initCall)write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) ,deltaAlphaA(1,3),deltaAlphaD(2,2) 
		if(LOUD.and.initCall)pause 'Wertheim: Asymptotic estimate. Calling XsolJre'
		call XsolJre2a(xFrac,rho,tKelvin,rdfContact,nComps,ierXsol)	 ! XA,XD,XC passed through module Assoc.
		if(LOUD.and.initCall)write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2):',XA(1,2),XA(1,3),XA(2,2),XD(1,2),XD(2,2) ,deltaAlphaA(1,3),deltaAlphaD(2,2) 
		if(LOUD.and.initCall)pause 'Wertheim: After Calling XsolJre'
		...
		DO iComp=1,nComps
			...
			do iType=1,nTypes(iComp)
				...
				if(WegFreq(iter).lt.0.01)then !check for iter hit.
					...
					if(ABS(dFa).gt.1e-11 .and. isEven(iter)==1)then
