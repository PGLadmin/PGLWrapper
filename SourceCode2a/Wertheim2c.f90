...
!	enddo
	...
	if(iDoSolvation==1)then  !checkup: 1E-111  ! Refine from ESD96 if necessary.
		...
		enddo
		if(LouderWert.and.initCall)write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(2,2),XD(2,3):',XA(1,2),XA(2,2),XD(2,3) ,deltaAlphaA(1,2),deltaAlphaD(2,2),deltaAlphaD(2,3) 
		if(LouderWert.and.initCall)pause 'Wertheim: Asymptotic estimate. Calling XsolJre'
		call XsolJre2a(xFrac,rho,tKelvin,rdfContact,nComps,ierXsol)	 ! XA,XD,XC passed through module Assoc.
		if(LouderWert.and.initCall)write(*,'(a,5F10.6,3F10.1)')' XA(1,2),XA(2,2),XD(2,3):',XA(1,2),XA(2,2),XD(2,3) ,deltaAlphaA(1,2),deltaAlphaD(2,2),deltaAlphaD(2,3) 
		if(LouderWert.and.initCall)pause 'Wertheim: After Calling XsolJre'
	...
	do while(rmsErr > 1E-6.and.iter < itMax)
		...
		DO iComp=1,nComps
			...
			do iType=1,nTypes(iComp)
				...
				if( (1+sumAcc) < 0) pause 'Wertheim: 1+sumAcc < 0'
				...
				if( (1+4*alphCCavg(iType)) < 0) pause 'Wertheim: 1+4*alphCCavg(iType) < 0'
