...
!c
      subroutine FuPrLorraine_TP(icon,ntyp,mtyp,tKelvin,pMPa,zn,rho,zFactor, &
...
!c----------------------------------------------------------
!c
      subroutine FuPrLorraine_TV(icon,tKelvin,vTotCm3,zn,pMPa,zFactor, &
     &                       fg,ft,fp,fx,uRes_RT,aRes_RT,CvRes_R,CpRes_R,dpdRho_RT)
...
!c-----------------------------------------------------------------------------
!c
      subroutine cubgen(ntyp,mtyp,t,p,xx,zz,fg,ft,fp,fx,aux)
!c
!c     parameters:
!c
...
!c-----dummy variables
      ...
      real(8) :: aux(15)
!c-----local variables
      integer :: i,j,k,n,ider,istab,icx,i3r
...
!c*** variables internes
!c
       real*8 pbsrt,bbb,ccc,del,eta2,A0,A1,A2,QQ,RR,TEST,S1,S2
       real*8 cur,pi,ZZ(3),ADD1,ADD2,theta,zmin,zmax
!c-----------------------------------------------------------------------------
...
!c
       ...
       if(dabs(eta1-8888d0).lt.1.0d-12) goto 100
  ...
  100  continue
       ...
       if(TEST.lt.0.0d0) then
          ...
          if(dabs(eta-zmin).lt.1.0d-12) eta1 = zmax
           ...
           eta1 = -0.5d0*(S1+S2) - A2 !since rop only converges to real solutions
                              !eta1 is assumed to be the real part of the
                              !imaginary solutions
           i3r = 2
           if(TEST > 1.0d-12) return
...
!c
 ...
 9000  format('phase initiale supposee monophasique, gm/rt=',d16.10)
...
!c
       if(nimp.ge.2) then
          ...
          write(lun1,9120) init,jd,etas(1),etas(2),fmaxi,sm1,testmax
          write(lun2,9120) init,jd,etas(1),etas(2),fmaxi,sm1,testmax
       endif
...
!c
 ...
 9050  format(1x,'constituant',8x,'moles',8x,'fractions molaires',15x &
     ...
     &                     ,(ki(j),j=1,iphase-1)
 9080     format(1x,a10,3x,f11.5,2x,f11.5,2x,f11.5,3x,g11.6)
       end do
 ...
 9140  format(1x,'Vol_tot/cm3  ',3x,g12.7,1x,g12.7)
 9141  format(1x,'Vol_tot/cm3  ',3x,g12.7,1x,g12.7,1x,g12.7, &
     &        2x,'drop out :',f6.2,' %')
 9142  format(1x,'vol_mol/cm3  ',3x,g12.7,1x,g12.7,1x,g12.7)
 9143  format(1x,'vol_mol/cm3  ',3x,g12.7,1x,g12.7)
