...
!c    Load BIP
      ...
      gE_file = '.\PrLorraineEtcFiles\' // trim(gE_file)
      open(50,file=(gE_file),status='old',err=10)
      do i = 1,nco-1
        do j = i+1,nco
           do
             read(50,*) id1,id2,BIP(1),BIP(2)
...
!c-------------------------------------------------------------------------------
!c
      subroutine therm(numero,tk,vol_mol,prop)

!c  compute thermal quantities as a function of temperature and molar volume
!c  inputs:
!c    numero--index of the pure fluid
!c        tk--temperature [K]
!c   vol_mol--molar volume [m3/mol]
!c  outputs:
!c        h--enthalpy [J/mol]                                  ----> prop(1)
!c        s--entropy [J/mol-K]                                 ----> prop(2)
!c        u--internal energy [J/mol]                           ----> prop(3)
!c       Cp--isobaric heat capacity [J/mol-K]                  ----> prop(4)
!c       Cv--isochoric heat capacity [J/mol-K]                 ----> prop(5)

      use comflash
      implicit none
      integer numero
      real*8 apr,bpr
      real*8 tk,vol_mol,daprdt,d2aprdt2,z0,pres0,h_res,pEoS,s_res
      real*8 a_res,u_res,cv_res,betap,kappav,cp_res
      real*8 h_ig,u_ig,s_ig
      real*8 a_function,f_cp,prop(nvarmax)

      common/ab_pr/apr,bpr

      apr = a_function(numero,tk,0)
      bpr = b(numero) * 1.0d-06

      pres0 = pEoS(tk,vol_mol)
      z0 = pres0 * vol_mol / (rgas * tk)
      daprdt = a_function(numero,tk,1) ! da / dT
      d2aprdt2 = a_function(numero,tk,2) !d2a / dT2

!c-----RESIDUAL PROPERTIES
!c     Residual molar Helmholtz energy
      a_res = rgas * tk * dlog(vol_mol/(vol_mol - bpr)) &
     &      + apr * dlog((vol_mol-bpr*r1)/(vol_mol-bpr*r2)) &
     &            / (bpr * (r1 - r2))
!c     Residual molar entropy
      s_res = rgas * dlog((vol_mol - bpr)/vol_mol) &
     &        - daprdt * dlog((vol_mol-bpr*r1)/(vol_mol-bpr*r2)) &
     &                 / (bpr * (r1 - r2))
!c     Residual molar internal energy
      u_res = a_res + tk * s_res
!c     Residual molar enthalpy
      h_res = u_res + pres0 * vol_mol - rgas * tk
!c     Residual Cv
      cv_res = - tk * d2aprdt2 * dlog((vol_mol-bpr*r1)/(vol_mol-bpr*r2)) &
     &              /  (bpr * (r1 - r2))

      betap = rgas / (vol_mol - bpr) - daprdt / (vol_mol-bpr*r1) &
     &                                        / (vol_mol-bpr*r2)
!c
      kappav = rgas * tk / (vol_mol - bpr)**2 &
     &       - apr * (2.0d0 * vol_mol - (r1+r2) * bpr) &
     &             / (vol_mol-bpr*r1)**2 / (vol_mol-bpr*r2)**2

      kappav = 1.0d0 / kappav

!c     Residual Cp
      cp_res = cv_res - rgas + tk * kappav * betap**2

!c     (dP/dT)@v = beta*P
      prop(dpt_id) = betap
!c     (dV/dP)@T = - kappa*v
      prop(dvp_id) = - kappav
!c     (dV/dT)@P = beta*P * kappa*v
      prop(dvt_id) = betap * kappav


!c-----VOLUME-TRANSLATION CORRECTED PROPERTIES
!c     Enthalpy
      h_res = h_res - pres0*parC(numero)
      prop(h_id) = h_res

!c     Joule-Thomson coefficient
      prop(jt_id) = (prop(dvt_id) * tk - vol_mol + parC(numero)) &
     &              / prop(cp_id)

!c     Volumetric expansivity
      prop(b_id) = prop(dvt_id) / (vol_mol - parC(numero))
...
!c ----------------------------------------------------------------------------
!c
      function f_cp(icomp,tk)
        use comflash
        implicit none
        real(8) :: tk,f_cp,coeff(nb_coeff),dippr_y
        integer :: icomp,eqId

        coeff(:) = tdep_prop(icomp,icp_idx,:)
        eqID = eqIDComp(icomp,icp_idx)

        f_cp = dippr_y(eqId,tc(icomp),coeff,tk)
        if(f_cp.gt.0.1d0*huge(1.0d0)) f_cp = 2.5d0*rgas*1d3
        f_cp = f_cp * 1d-3 ! J K -1 kmol-1 > J K mol-1

      end function
!c
!c ----------------------------------------------------------------------------
!c
      function f_cpsT(icomp,tk)
        use comflash
        implicit none
        real(8) :: tk,f_cpsT,coeff(nb_coeff),dippr_y
        integer :: icomp,eqID

        coeff(:) = tdep_prop(icomp,icp_idx,:)
        eqID = eqIDComp(icomp,icp_idx)

        f_cpsT = dippr_y(eqId,tc(icomp),coeff,tk)
        if(f_cpsT.gt.0.1d0*huge(1.0d0)) f_cpsT = 2.5d0*rgas*1d3
        f_cpsT = f_cpsT * 1d-3 ! J K -1 kmol-1 > J K mol-1
        f_cpsT = f_cpsT / tk
      end function
!c
!c ----------------------------------------------------------------------------
...
!c-------------------------------------------------------------------------------
!c
      subroutine calcul_TEos(numero,pres,vol_mol,tk,ierr)
        use comflash
        implicit none
        integer numero,ierr,niter
        real*8 pres,vol_mol,tk,t_old,fp,eps,deltat,deltap
        real*8 a_function,pEoS,dpsdt,daprdt
        real*8 apr,bpr,prop(nvarmax)
        common/ab_pr/apr,bpr

        eps = 1.0d-10
        pres = pres * 1.d5 ! bar > Pa

        tk = pres * vol_mol / rgas !Initialization ideal gas
        bpr = b(numero) * 1.d-6 ! cm3/mol > m3/mol
        ierr = 0
        niter = 0
        do
           t_old = tk
           apr = a_function(numero,t_old,0)
           daprdt = a_function(numero,t_old,1)
           fp = pEoS(t_old,vol_mol) - pres
           call therm(numero,t_old,vol_mol,prop)
           dpsdt = prop(dpt_id)
           tk = t_old - fp / dpsdt
           deltat = dabs((tk - t_old) / tk)
           deltap = dabs(fp/pres)
           niter = niter + 1
           if(deltat.lt.eps .and. deltap.lt.eps) exit
           if(deltat.lt.1.d-14 .or. deltap.lt.1.d-14) exit
           if(niter.gt.nitflash) then
              ierr = 1
              exit
           end if
        end do
        pres = pres * 1.d-5 ! Pa > bar
        return
      end subroutine
!c
!c-------------------------------------------------------------------------------
!c
      subroutine calcul_TEos_POspec(numero,pres,ospec,o_idx,vol_mol, &
     &                              tk,ierr)
!c  iterate for single-phase temperature as a function of (enthalpy or entropy or
!c  internal energy), pressure, and composition
        use comflash
        implicit none
        integer numero,ierr,o_idx,err1,niter
        real*8 pres,vol_mol,tk,t_old,fp,eps,deltat,deltao
        real*8 dosdt,ospec,a_step,step
        real*8 apr,bpr,prop(nvarmax)
        common/ab_pr/apr,bpr
        ...
        t_old = tk
        step = 1.0d-1
        call dichotomie_T(numero,step,pres,t_old,vol_mol,o_idx,ospec)
        tk = t_old
        !Newton loop
        do
           t_old = tk
           call calcul_racine_stable_pur(numero,t_old,pres,vol_mol,err1)
           if(err1.ne.0)then
              ierr = 2
              exit
           end if

           call therm(numero,tk,vol_mol,prop)
           fp = prop(o_idx) - ospec
           if(o_idx.eq.1) then
              dosdt = prop(cp_id) ! (dh/dT)@P = Cp
           elseif(o_idx.eq.2) then
              dosdt = prop(cp_id) / t_old ! (ds/dT)@P = Cp / T
           elseif(o_idx.eq.3) then
              dosdt = prop(cp_id) - pres*1.0d05 * prop(dvt_id) ! (dU/dT)@P = Cp - P(dV/dT)@P
           end if
           a_step = 1.0d0
           step = a_step * fp / dosdt
           do while(step.gt.t_old)
              a_step = a_step * 0.5d0
              step = a_step * fp / dosdt
           end do
           tk = t_old - fp / dosdt
           deltat = dabs((tk - t_old) / tk)
           deltao = dabs(fp/ospec)
           niter = niter + 1

           if(deltat.lt.eps .and. deltao.lt.eps) exit
           if(deltat.lt.1.d-14 .or. deltao.lt.1.d-14) exit
           if(niter.gt.nitflash) then
              ierr = 1
              exit
           end if
...
!c-------------------------------------------------------------------------------
!c
      subroutine calcul_PEos_TOspec(numero,p_in,ospec,o_idx,vol_mol, &
     &                              tk,ierr)
!c  iterate for single-phase temperature as a function of (enthalpy or entropy or
!c  internal energy), pressure, and composition
        use comflash
        implicit none
        integer numero,ierr,o_idx,err1,niter
        real*8 pres,p_in,vol_mol,tk,p_old,fp,eps,deltap,deltao
        real*8 dosdp,ospec,step,a_step
        real*8 apr,bpr,prop(nvarmax)
        common/ab_pr/apr,bpr
        ...
        do
           p_old = pres !bar
           call calcul_racine_stable_pur(numero,tk,p_old,vol_mol,err1)
           if(err1.ne.0)then
              ierr = 2
              exit
           end if
           ...
           fp = prop(o_idx) - ospec
           if(o_idx.eq.1) then
              dosdp = vol_mol - tk * prop(dvt_id) ! (dh/dP)@T = V - T(dV/dT)@P
           elseif(o_idx.eq.2) then
              dosdp = -prop(dvt_id)               ! (ds/dP)@T = -(dV/dT)@P
           elseif(o_idx.eq.3) then
              dosdp = -tk * prop(dvt_id) - pres*1.0d05 * prop(dvp_id) ! (dU/dP)@T = -T(dV/dT)@P - P(dV/dP)@T
           end if
           a_step = 1.0d0
           step = a_step * fp / dosdp
           do while(step.gt.p_old*1.0d05)
              a_step = a_step * 0.5d0
              step = a_step * fp / dosdp
           end do
           pres = p_old*1.0d05 - a_step * (fp / dosdp) ! Pa
           pres = pres * 1.0d-05 !Pa > bar
           deltap = dabs((pres - p_old) / pres)
           deltao = dabs(fp/ospec)
           niter = niter + 1

           if(deltap.lt.eps .and. deltao.lt.eps) exit
           if(deltap.lt.1.d-14 .or. deltao.lt.1.d-14) exit
           if(niter.gt.nitflash) then
              ierr = 1
              exit
           end if
...
!c---------------------------------------------------------------------------
!c
      subroutine calcul_TPEos_VOspec(numero,pres,ospec,o_idx,vol_mol, &
     &                              tk,ierr)
!c  iterate for single-phase temperature as a function of (enthalpy or entropy or
!c  internal energy), volume, and composition
        use comflash
        implicit none
        integer numero,ierr,o_idx,niter
        real*8 pres,vol_mol,tk,t_old,fp,eps,deltat,deltao
        real*8 a_function,pEoS,dosdt,ospec,a_step,step
        real*8 apr,bpr,prop(nvarmax)
        common/ab_pr/apr,bpr
        ...
        do
           ...
           fp = prop(o_idx) - ospec
           if(o_idx.eq.1) then
              dosdt = prop(cv_id) + vol_mol * prop(dpt_id) ! (dh/dT)@V = Cv + v(dP/dT)@V
           elseif(o_idx.eq.2) then
              dosdt = prop(cv_id) / t_old ! (ds/dT)@V = Cv / T
           elseif(o_idx.eq.3) then
              dosdt = prop(cv_id)  ! (dU/dT)@V = Cv
           end if
           a_step = 1.0d0
           step = a_step * fp / dosdt
           do while(step.gt.t_old)
...
!c---------------------------------------------------------------------------
!c
      subroutine calcul_TP_2PH_VOspec(numero,pres,ospec,o_idx,vol_mol, &
     &                              tk,vliq,vvap,ierr)
!c  iterate for saturation temperature as a function of (enthalpy or entropy or
!c  internal energy), volume, and composition
        use comflash
        implicit none
        integer numero,ierr,o_idx,err1,niter
        real*8 pres,vol_mol,tk,t_old,fp,eps,deltat,deltao
        real*8 dosdt,ospec,a_step,step
        real*8 qx,vliq,vvap,dxsdt,dvapo,dvapv,dvaps,dolsdt,dovsdt
        real*8 dvlsdt,dvvsdt,teb
        real*8 apr,bpr,prop(nvarmax),prop_l(nvarmax),prop_v(nvarmax)
        common/ab_pr/apr,bpr
        ...
        do
           t_old = tk
           call calcul_psat(numero,t_old,pres,err1,vliq,vvap)
           if(err1.ne.0)then
              ierr = 2
              exit
           end if

           !Calculation of properties for the liquid root
           call therm(numero,t_old,vliq,prop_l)
           !Calculation of properties for the vapor root
           call therm(numero,t_old,vvap,prop_v)

           !Compute vapor quality
           qx = (vol_mol - vliq) / (vvap - vliq)

           dvapo = prop_v(o_idx) - prop_l(o_idx)
           dvaps = prop_v(s_id) - prop_l(s_id)
           dvapv = vvap - vliq

           prop(o_idx) = prop_l(o_idx) + dvapo * qx

           fp = prop(o_idx) - ospec

           !Derivative calculation (dx/dT)@V in the two-phase region
           !Set: x = (v - v_L) / (v_V - v_L)
           !(dx/dT)@V = - (1.0 / dvap_V) * ( x*(dV_V/dT)@ELV + (1 -x) * (dV_L/dT)@ELV)
           !(dV/dT)@ELV = (dV/dT)@P + (dvap_S / dvap_V) * (dV/dP)@T

           dvlsdt = prop_l(dvt_id) + (dvaps / dvapv) * prop_l(dvp_id)
           dvvsdt = prop_v(dvt_id) + (dvaps / dvapv) * prop_v(dvp_id)
           dxsdt = - (1.0d0 / dvapv) * (qx * dvvsdt + &
     &                                          (1.0d0 - qx) * dvlsdt)


           !Derivative calculation (dO/dT)@V in the two-phase region
           !Set: O = O_L + dvap_O * qx; with O = Enthalpy, entropy or internal energy
           !(dO/dT)@V = (dO_L/dT)@VLE *(1.0 - x) + (dO_V/dT)@VLE * x + dvap_O * (dqx/dT)@V

           if(o_idx.eq.1) then
              !(dh/dT)@ELV = Cp + (dvap_S / dvap_V) * (v - T * (dV/dT)@P)
              dolsdt = prop_l(cp_id) + (dvaps / dvapv) * &
     &                (vliq - t_old * prop_l(dvt_id))
              dovsdt = prop_v(cp_id) + (dvaps / dvapv) * &
     &                (vvap - t_old * prop_v(dvt_id))
           elseif(o_idx.eq.2) then
              !(ds/dT)@ELV = Cp / T - (dvap_S / dvap_V) * (dV/dT)@P
              dolsdt = prop_l(cp_id) / t_old + (dvaps / dvapv) * &
     &                 prop_l(dvt_id)
              dovsdt = prop_v(cp_id) / t_old + (dvaps / dvapv) * &
     &                 prop_v(dvt_id)
           elseif(o_idx.eq.3) then
              !(du/dT)@ELV = Cp - P * (dV/dT)@P - (dvap_S / dvap_V) * ((dV/dT)@P + P * (dV/dP)@T)
              dolsdt = prop_l(cp_id) - pres * 1.0d05 * prop_l(dvt_id) &
     &          - (dvaps / dvapv) * (t_old * prop_l(dvt_id) &
     &                             + pres * 1.0d05 * prop_l(dvp_id))
              dovsdt = prop_v(cp_id) - pres * 1.0d05 * prop_v(dvt_id) &
     &          - (dvaps / dvapv) * (t_old * prop_v(dvt_id) &
     &                             + pres * 1.0d05 * prop_v(dvp_id))
           end if
           dosdt = qx * dovsdt + (1.0d0-qx) * dolsdt + dvapo * dxsdt
           a_step = 1.0d0
           step = a_step * fp / dosdt
           do while(step.gt.t_old)
              a_step = a_step * 0.5d0
...
!c-------------------------------------------------------------------------------
!c
      subroutine calcul_teb_v(numero,v_in,teb,psat,err0,vol_mol_liq, &
     &                                                     vol_mol_vap)
...
!c
!c INPUT : numero, entier = 1 ou 2 pour spécifier le constituant
!c         v_in (volume molaire specifie) en mol/m3
!c OUTPUT :  teb (K), err0 (variable d'erreur, = 0 si tout va bien)
!c           psat (bar)
!c
      use comflash
      implicit none
      real*8 apr,bpr,pcrit,tcrit,vcrit
      real*8 teb,teb0,psat,v_in,vol_mol_liq,vol_mol_vap,vol_mol
      real*8 pbar,fp,dvdt
      real*8 res1,res2,eps,a_step,step
      real*8 prop(nvarmax),prop_l(nvarmax),prop_v(nvarmax),vvap,svap

      integer numero,nb_iter,nb_iter_max,err0
      common/ab_pr/apr,bpr
      ...
      !Calculer vcrit
      call calcul_racine_stable_pur(numero,tcrit,pbar,vcrit,err0)

      if(dabs(vcrit - v_in) / v_in.lt.1.d-5) then
         teb = tcrit
         psat = pbar
         goto 31
      endif
...
!c     -----------------------------------
 30   continue
      teb0 = teb
      call calcul_psat(numero,teb0,psat,err0,vol_mol_liq,vol_mol_vap)

      if(err0.ne.0) goto 31

      call therm(numero,teb0,vol_mol_liq,prop_l)
      call therm(numero,teb0,vol_mol_vap,prop_v)

      svap = prop_v(s_id) - prop_l(s_id)
      vvap = vol_mol_vap - vol_mol_liq

      if(v_in.lt.vcrit) then
          vol_mol = vol_mol_liq
          prop(1:nvarmax) = prop_l(1:nvarmax)
      endif
      if(v_in.gt.vcrit) then
          vol_mol = vol_mol_vap
          prop(1:nvarmax) = prop_v(1:nvarmax)
      end if

      fp = vol_mol - v_in
      dvdt = prop(dvt_id) + prop(dvp_id) * svap / vvap

       a_step = 1.0d0
       step = a_step * fp / dvdt
       do while(step.gt.teb0)
          a_step = a_step * 0.5d0
          step = a_step * fp / dvdt
       end do

      teb = teb0 - a_step * fp / dvdt

      if(nb_iter.eq.nb_iter_max) then
         eps = 1.d-6

         if(res1.lt.eps .and. res2.lt.eps) goto 31
         err0 = 2
         goto 31
      endif
      nb_iter = nb_iter + 1

      res1 = dabs(vol_mol - v_in)/dabs(v_in)
      res2 = dabs(teb - teb0)/dabs(teb0)

      if(res1.lt.eps .and. res2.lt.eps &
     &   .and. nb_iter>1) then
         goto 31
      else
         goto 30
...
!c
      subroutine fugacity_TP(icon,ntyp,mtyp,tKelvin,pMPa,zn,rho,zFactor, &
     &                       fg,ft,fp,fx,uRes_RT,aRes_RT,CvRes_R,CpRes_R,dpdRho_RT)

      use comflash
      implicit none
      integer :: icon,ntyp,mtyp
      real(8) :: tKelvin,pMPa,zn(ncomax),rho,zFactor,uRes_RT
...
!c-----------------------------------------------------------------------------
      ...
      call cubgen(ntyp,mtyp,tKelvin,pMPa,zn,zFactor,fg,ft,fp,fx,aux)
...
!c----------------------------------------------------------
!c
      subroutine fugacity_TV(icon,tKelvin,vTotCm3,zn,pMPa,zFactor, &
     &                       fg,ft,fp,fx,uRes_RT,aRes_RT,CvRes_R,CpRes_R,dpdRho_RT)
...
!c-----------------------------------------------------------------------------
!c
      subroutine cubgen(mt,ic,t,p,xx,zz,fg,ft,fp,fx,aux)
!c
!c     parameters:
!c
...
!c-----dummy variables
      ...
      real(8) :: aux(15)
!c-----local variables
      integer :: i,j,k,n,ider,istab,ierr,icx
      real(8) :: v0,lnphi_c(ncomax)
      real(8) :: x(ncomax),pd(ncomax),ad1(ncomax),adt(ncomax)
...
!c
      ...
      integer :: ider
      real(8) :: t,vv,p,pn,som,sumn,vv2,aux(15)
...
!c------ -------excess Helmholtz energy/RT stored in aux(12)
                ...
                ft(1:n) = dmudt(1:n) - lnphi_c(1:n) * (dpdt/peq + 1.0d0/t)    !d ln phi(i) / dT
...
!c                             de T ou pour calculer prop dérivées
       ...
       real*8    bxen,aapur,sumb(ncomax),unspuisb, truc,pro
...
!c      Pour règles de mélange NRTL
       ...
       real*8    dEijT_sur_Eij(ncomax,ncomax),dei_dT(ncomax),dasb_dT(ncomax)
...
!c
     ...
     &           dalpha_dT*qp*(derbm(i)/bm - 1.0d0/z) - q*daai_dT(i)
          ...
          do k=1,nco
             dmudn(i,k) = dlphidx(i,k) - somd ! d ln phi(i) / dnk
          end do
!c
       end do

       !Volume-translation corrections
...
!c
 10    etasb = eta/bm
       ...
       do i=1,nco
          ...
          do k=1,nco
            dmudn(i,k) = dlphidx(i,k) - somd + eta*dlphideta(i) ! d ln phi(i) / dnk
          end do
...
!c
      ...
      integer i,j
      real*8  xcomp(nco),tk,volmol,hec,aec,sec
...
!c*****************************************************************************
      subroutine integral_cp(av,bv,icomp,integral)
        use comflash
        implicit none
        integer :: i,icomp
        real(8) :: zz(10),w(10)
        real(8) :: av,bv,x,integral,somme,f_cp
        ...
        somme = 0.0d0
        do i = 1,10
            x = (av+bv)/2.0d0 + (bv-av)*zz(i)/2.0d0
            somme = somme + w(i)*f_cp(icomp,x)
        end do

        integral = (bv-av)*somme/2.0d0

      end subroutine
!C
!C-------------------------------------------------------
!C
      subroutine integral_cpsT(av,bv,icomp,integral)
        use comflash
        implicit none
        integer :: i,icomp
        real(8) :: zz(10),w(10)
        real(8) :: av,bv,x,integral,somme,f_cpsT
        ...
        do i = 1,10
            x = (av+bv)/2.0d0 + (bv-av)*zz(i)/2.0d0
            somme = somme + w(i)*f_cpsT(icomp,x)
        end do
...
!-------------------------------------------------------------
      subroutine dichotomie(icomp,step,vk,tk)
        use comflash
        implicit none
        real(8) :: avalue,bvalue
        real*8 step,y0,vk,tk,prop(nvarmax)
        real(8) :: prec
        integer :: icomp
!--------------------------------------------------------------
        prec = 1d-5
        call therm(icomp,tk,vk,prop)
        y0 = prop(jt_id)
        if (y0 <0.0d0) then
            avalue = vk
            do while (y0 < 0.0d0)
         ...
         do while (dabs(avalue - bvalue) > prec)
            vk = (avalue + bvalue) / 2.0d0
            call therm(icomp,tk,vk,prop)
            y0 = prop(jt_id)
            if (y0 < 0.0d0) then
                avalue = vk
...
!-------------------------------------------------------------
      subroutine dichotomie_T(icomp,step,pk,tk,vk,o_id,o_val)
        use comflash
        implicit none
        real(8) :: avalue,bvalue
        real*8 step,y0,vk,tk,pk,prop(nvarmax)
        real(8) :: prec,o_val
        integer :: icomp,o_id,err1
!--------------------------------------------------------------
        prec = 1d-4
        call calcul_racine_stable_pur(icomp,tk,pk,vk,err1)
        call therm(icomp,tk,vk,prop)
        y0 = prop(o_id) - o_val
        if (y0 <0.0d0) then
            avalue = tk
