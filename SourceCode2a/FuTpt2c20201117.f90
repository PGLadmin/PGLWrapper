...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    ...
    DO i=1,nComps
        ...
        if(tKelvin < tKmin(i))then	 ! Don't round Tmin down here or NUMDERVS may fail later.
            iErr=5
            if(LOUD.and.initCall)write(*,*)'FuTpt:iComp,TKmin= ',i,TKmin(i),TRIM( errMsg(iErr) )
        endif
        if(tKelvin < tKmin(i))isTtooLow=1	! Don't round Tmin down here or NUMDERVS may fail later.
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	errMsg(5)=' FuTptVtot Error: T < tKmin for at least one component.'
    ...
    DO i=1,nComps
		...
		bRef(I)=bVolRef(i,tKelvin) !Function bVolRef()
        bRefMix=bRefMix+xFrac(i)*bRef(i)
        if(tKelvin < tKmin(i)*0.998D0)then	   !round TKmin down a little for possible lost precision
            iErr=5
            if(LOUD)write(*,*)'FuTptVtot:iComp,T(K),TKmin= ',i,tKelvin,tKmin(i),TRIM( errMsg(iErr) )
        endif
        !chemPo(iComp)=86 !initialize to error indicator in case of bad return !No chemPo here !!!
    enddo
    hRes_RT=86.8686D0 !initialize to error indicator in case of bad return
    ...
    Sres_R =86.8686D0 !initialize to error indicator in case of bad return
    zFactor=86.8686D0 !initialize to error indicator in case of bad return
    if(iErr.ne.0 .and. iErr.ne.5)return	  ! let iErr==5 be a warning. e.g. liquid density may be ok although Psat would be bogus.  
    !bVolPure=bVolMix
	...
	if(iErr.eq.0)call Wertheim(vMolecNm3,eta,tKelvin,xFrac,nComps,zAssoc,aAssoc,uAssoc,iErrCode)
