!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	USE GlobConst, only: avoNum,zeroTol,bVolCC_mol,ID
	!USE SpeadParms
	USE Assoc ! XA,XD,XC,NAS(NMX),NDS(NMX),ND(NMX),...
	!  PURPOSE:  COMPUTE THE EXTENT OF ASSOCIATION (fAssoc) AND zAssoc given rho,VM
	Implicit DoublePrecision(A-H,K,O-Z)
	...
	!  NDi     = THE DEGREE OF POLYMERIZATION OF COMPO i
	!  XB	   = Fraction of site type B that is unbonded (B= A,D,or C). C is for carboxylic acids.
	!  FA      = THE CHARACTERISTIC ASSOCIATION = (1/XAi-1)/RALPHi
	!  RALPHi  = ROOT OF ALPHA WHERE ALPHAi=rho*bVoli*KADi*Ei*rdfContact
	!  
	iErr=0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
	subroutine Wertheim( isZiter,vMolecNm3,eta,tKelvin,xFrac,nComps,zAssoc,aAssoc,uAssoc,rLnPhiAssoc,iErrCode)
	...
	DoublePrecision vMolecNm3(NMX),xFrac(NMX),ralph(NMX,maxTypes),rLnPhiAssoc(NMX) !bondVolNm3(NMX),
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1111111111
	...
	!
	!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC  
	SUBROUTINE WertheimFugc(xFrac,vMolecNm3,tKelvin,nComps,ETA,FUGASSOC,h_nMichelsen,dFUGASSOC_dT,dFUGASSOC_dRHO,dh_dT,dh_dRHO)
	USE GlobConst, only: avoNum,zeroTol,bVolCC_mol,ID
	USE Assoc
	!USE BIPs
	IMPLICIT DOUBLEPRECISION(A-H,O-Z)
	LOGICAL LOUDER
	DIMENSION xFrac(NMX),vMolecNm3(NMX),FUGASSOC(NMX) !,NAS(NMX),NDS(NMX),ND(NMX),ralph(NMX),XAmol(NMX),XDmol(NMX)
	...
	needDerivs=0
	if(needDerivs==0)return	! Let's forget about derivatives for now and stick to VLE calculations.
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	do iComp=1,nComps		 
		...
		do iType=1,nTypes(iComp)
			...
			dFUGASSOC_dRHO(iComp)=dsumLnXi_dRHO-half*((dalpha-1.d0)*bVolCC_mol(iComp)/bVolMix*dh_dRHO+h_nMichelsen*dAlph_deta*vMolecNm3(iComp))
		enddo	
	enddo
