...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if(init.EQ.2)THEN
		...
		if(ierFugi(1) > 0)then
			ier(1) = 13
			if(LOUDER)write(*,*)' BUBPL: error from Fugi on vapor initialization. ierF(1)=',ierFugi(1)
			return
		endif
	END if
	kLiq=1
	...
	DO WHILE(ier(1) < 0)
		...
		if(kLiq.NE.0)THEN
			...
			if(ierFugi(1).NE.0) THEN
				...
				ENDDO
				ier(1)=ier(4)
				if(LOUDER)write(*,*)' BUBPL: error from Fugi of liquid. ierF(1)=',ierFugi(1)
		...
		if( iter > 1 ) THEN                 !  FOR HIGHER iterATIONS, A SECANT STEP IS TAKEN.
			if(gObj==gOld)THEN			
				ier(7)=1
				if(LOUDER)write(*,*)' BUBPL: fNew=fOld=',gOld
				GOTO 86
			END if
			...
			if (pMpa < 0) THEN
				pMpa=EXP(pMpa)
				IF(LOUDER)WRITE(6,*)' BUBPL WARNING: P IS -VE IN BUBPL. SHORTENING STEP.'
			ENDif
		...
		if(ierFugi(1).NE.0)THEN
			DO iErrCheck=4,11
				if( ierFugi(iErrCheck) == 1)ier(4)=iErrCheck
			enddo
			if(LOUDER)write(*,*)' BUBPL: error from Fugi on vapor iteration. ierF(1)=',ierFugi(1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	DO I=1,NC
		IF(TC(I) < 0)IER(8)=1
		IF(PC(I) < 0)IER(8)=1
		IF(X(I) < 0 .OR. X(I) > 1)IER(8)=0
	enddo
	IF(P < 0)IER(9)=1
	IF(INIT==1 .AND. T < 0)IER(10)=1
	IF(NC > 10)IER(11)=1
	IF(ITMAX < 1)IER(12)=1
	DO I=8,12
...
13		continue !re-entry for bad T guess     
		...
		IF(KLIQ.NE.0)THEN
			...
			etaL=etaPass
			IF(ierf(1).NE.0.AND.ITER > 1)THEN
				DO IE=4,11
					IF(ierf(IE)==1)IER(4)=IE
				enddo
		...
		IF(ITER==1)THEN
			...
			TI=TI*1.01
			T=1/TI
			IF(ZL > 0.33.OR.ierf(1).NE.0)then
				iCount=iCount+1
				!c		  WRITE(*,*)'TI,T',TI,T
		...
		IF(DABS(CHNG/TI) > 0.02D0)CHNG=DSIGN(0.02D0,CHNG)*TI
		TINEW=TI - CHNG
		TI=TINEW
...
25		continue !skip to point for 1st iteration
		...
		T=1/TI
		LIQ=0
		if(iter > 1)CALL Fugi(T,P,Y,NC,LIQ,FUGCV,ZV,ierf)
		etaV=etaPass
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	doublePrecision calck(NMX),xFrac(NMX),yFrac(NMX),zFeed(NMX),xTemp(NMX)
	...
	if(iPhaseUp.eq.1)then !compute fugcPure to be sure gEx > 0
		do iComp=1,nComps
			xTemp(iComp)=xInf
		enddo
		do iComp=1,nComps
			xTemp(iComp)=1
			CALL Fugi(tKelvin,pMpa,xTemp,nComps,1,fugcLiq,zLiq,ierFugi)
	...
	DO while(iErrCode.eq.0.and.iFlag.ne.0) !iFlag checks convergence of compositions
		...
		DO iComp=1,nComps
			...
			IF (xFrac(iComp) < 0)then
				iErrCode=7
				goto 86
			endif
        enddo
        xFrac(1:nComps)=xFrac(1:nComps)/sum( xFrac(1:nComps) )
        yFrac(1:nComps)=yFrac(1:nComps)/sum( yFrac(1:nComps) )
		if(ierVof.ne.1)iTry=iTry+1
		IF(ierVof==3.and.iTry > 2)then
			iErrCode=2
			GOTO 86
		ELSE IF(ierVof==2.and.iTry > 2)then
			iErrCode=3
			GOTO 86
		END IF
		...
		if(picardFactor > 1)picardFactor=1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	USE GibbsMin
	IMPLICIT DOUBLEPRECISION(A-H,O-Z)
	DoublePrecision fugcLo(NMX),fugcUp(NMX),gamInf(NMX),rLnGam(NMX) !,actCo(NMX),Gmix(21),dG_dx1(21),d2G_dx12(21)
	DoublePrecision X(nmx),XU(nmx),ZFEED(nmx),calcK(nmx),calcKold(nmx),LLTol,GmixMinusLine !,GPure(nmx)
	External GmixMinusLine
	common/EtaLiqPas/etaPasLo,etaPasUp,zPasLo,zPasUp
	!if(initialize==1)then !Use the input calcK's for initial guess.
	!if(initialize> 1)then !check d2G/dx1^2 < 0 somewhere and spinodal,maxGams for initial guess.
	! LLSpinodals computes gamInf and Gpure ase well as other properties. They are USEd in GibbsMin.
	!if(initialize==3)then ! pick x1estLo and x1estHi from tangent line.
	!integer ierFugi(12)
	character*77 errMsg(0:22)
	LOGICAL LOUDER
	!Character*1 answer
	!COMMON/FEED/ZFEED
	iErrCode=0
...
611 format(1x,a,i8,12E12.4)
	tol=1.d-4
	errMsg(0 )='LLEFL: Success!'
	errMsg(11)='LLEFL: (K1-K2) ~ 0 in denom during flash iteration.'
	errMsg(12)='LLEFL: Error from LLSpinodal.'
	errMsg(13)='LLEFL: x1Up~x1Lo from LLSpinodal. No LLE?'
	errMsg(15)='LLEFL: FugiTP error during flash iteration.'
	errMsg(16)='LLEFL: itMax exceeded.'
	if(LOUDER)write(dumpUnit,*)'LLEFL: Starting with INIT,ITMAX=',initialize,itMax


10	tKelvin=T	!tKelvin is USEd in GibbsMin
	pMPa=P		!pMPa is USEd in GibbsMin
	if(NC == 2)then	 !compute gMix to be sure gEx > 0
		if(initialize > 1)then !check d2G/dx1^2 < 0 somewhere and spinodal,maxGams for initial guess.
			! LLSpinodals computes gamInf and Gpure ase well as other properties. They are USEd in GibbsMin.
			Call LLSpinodals(NC,gamInf,x1SpinLo,GSpinLo,x1SpinHi,GSpinHi,x1minLo,GmixMinLo,x1minHi,GmixMinHi,x1con,d2G_dX1con,iErrSpin) 
			if(iErrSpin > 9)then
				if(LOUDER)write(dumpUnit,611)'LLEFL: Spinodal Error=',iErrSpin
				iErrCode=12
				return  
			elseif(d2G_dX1con > 0) then
				if(LOUDER)write(dumpUnit,610)'LLEFL: no apparent LLE. x1SpinLo,x1SpinHi,min(d2G/dX1^2)=',x1SpinLo,x1SpinHi,d2G_dX1con
				iErrCode=13
				return
			elseif( ABS(x1SpinHi-x1SpinLo) < 0.01d0)then
				if(LOUDER)write(dumpUnit,610)'LLEFL: questionable LLE. x1SpinLo,x1SpinHi,min(d2G/dX1^2)=',x1SpinLo,x1SpinHi,d2G_dX1con
				x(1)=x1SpinLo
				xU(1)=x1SpinHi
				x(2)=1-x(1)
				xU(2)=1-xU(1)
				iErr=4 ! leave it as a warning.
				return
			elseif(gamInf(1) > 20 .and. gamInf(2) > 20)then ! we don't need to search for tangent line in this case.
				x(1) =1/gamInf(1)
				x(2)=1-x(1)
				xU(2)=1/gamInf(2)
				xU(1)=1-xU(2)
			...
			if(initialize==3)then ! pick x1estLo and x1estHi from tangent line.
				...
				x1Lo=1.1D-4 ! must be bigger than xStep
				x1Hi=1-x1Lo
				tolTan=tol/5
				if(GmixMinLo > GmixMinHi)then
					x1Min=x1MinHi
					Gmin=GmixMinHi
					constant=0
					slope=GmixMinHi/x1MinHi
					Call GoldenMin(GmixMinusLine,x1Lo,x1spinLo,tolTan,x1estLo,GestLo,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
					GestLo=GestLo+constant+slope*x1estLo ! GestLo from GmixMinusLine needs correction by PlusLine
					if(LOUDER)write(dumpUnit,610)'LLEFL: INIT=3, x1estLo,GestLo=',x1estLo,GestLo
					slope=(Gmin-GestLo)/(x1Min-x1estLo)
					constant=GestLo-slope*x1estLo
					Call GoldenMin(GmixMinusLine,x1spinHi,x1Hi,tolTan,x1estHi,GestHi,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
					GestHi=GestHi+constant+slope*x1estHi ! GestHi from GmixMinusLine needs correction by PlusLine
					!Refine GestLo
					slope=(GestHi-GestLo)/(x1estHi-x1estLo)
					constant=GestHi-slope*x1estHi
					Call GoldenMin(GmixMinusLine,x1Lo,x1spinLo,tolTan,x1estLo,GestLo,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
					GestLo=GestLo+constant+slope*x1estLo ! GestHi from GmixMinusLine needs correction by PlusLine
					if(LOUDER)write(dumpUnit,610)'LLEFL: INIT=3, x1estLo,GestLo=',x1estLo,GestLo
					dG_dX1Lo=dG_dX1	!USEd in GibbsMin, Computed from rLnGam.
					if(LOUDER)write(dumpUnit,610)'LLEFL: slope,dG/dX1Lo,dG/dX1Hi=',slope,dG_dX1Lo,dG_dX1Hi
				else  !	GmixMinLo < GmixMinHi
					x1Min=x1MinLo
					Gmin=GmixMinLo
					slope=(0-GmixMinLo)/(1-x1minLo)
					!GLine=constant+slope*x1 = GmixMinLo @ x1minLo => c = GmixMinLo-slope*x1minLo
					constant=GmixMinLo-slope*x1minLo
					Call GoldenMin(GmixMinusLine,x1SpinHi,x1Hi,tolTan,x1estHi,GestHi,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
					GestHi=GestHi+constant+slope*x1estHi ! GestHi from GmixMinusLine needs correction by PlusLine
					if(LOUDER)write(dumpUnit,610)'LLEFL: INIT=3, x1estHi,GestHi=',x1estHi,GestHi
					dG_dX1Hi=dG_dX1	!USEd in GibbsMin, Computed from rLnGam.
					slope=(Gmin-GestHi)/(x1Min-x1estHi)
					constant=GestHi-slope*x1estHi
					Call GoldenMin(GmixMinusLine,x1Lo,x1spinLo,tolTan,x1estLo,GestLo,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
					GestLo=GestLo+constant+slope*x1estLo ! GestHi from GmixMinusLine needs correction by PlusLine
					if(LOUDER)write(dumpUnit,610)'LLEFL: INIT=3, x1estLo,GestLo=',x1estLo,GestLo
		...
		elseif(initialize==1)then
			...
			xU(1)=x(1)*calcK(1)
			if(x(1) < zeroTol .or. x(1) > 1)then
				if(LOUDER)pause 'LLEFL: x1Lo out of range during initialization.'
				initialize=2
				goto 10
			endif
			if(xU(1) < zeroTol .or. xU(1) > 1)then
				if(LOUDER)pause 'LLEFL: x1Up out of range during initialization.'
				initialize=2
				goto 10
			endif
		endif !initialize==1
...
100		continue
		...
		alpha=0.6d0  ! enable Picard iteration.
		x1Lo=x(1)
		!x1Up=spinHi*alpha+(1-alpha)*1
		!x(1)=x1Lo
		x1Up=xU(1)
		calcKold(1:2)=1 !initialize
		if(LOUDER)write(dumpUnit,610)'LLEFL: Initial T,x1Lo,x1Up =',T,x1Lo,x1Up

		if(LOUDER)print*,'    x1       x1U      calcK1      calcK2      x1new    x1Upnew'
		LLTOL=1.D-3
		if(LOUDER)write(dumpUnit,*)'LLEFL: itMax,INIT=',itMax,initialize
		...
		do iter=1,itMax
			...
			CALL FugiTP(T,P,x ,NC,1,rhoLo,zLo,aRes,fugcLo,uRes,iErrF)
			if(iErrF > 9)iErrCode=15
			rLnGam(1:NC)=fugcLo(1:NC)-Gpure(1:NC)
			CALL FugiTP(T,P,xU,NC,1,rhoUp,zUp,aRes,fugcUp,uRes,iErrF)
			if(iErrF > 9)iErrCode=15
			if(iErrCode > 9)then
				if(LOUDER)write(dumpUnit,*)TRIM(errMsg(15))
				exit
			endif
			calcK(1:NC)=EXP(fugcLo(1:NC)-fugcUp(1:NC))
			calcK(1:NC)=EXP( rLnGam(1:NC)-(fugcUp(1:NC)-Gpure(1:NC)) )
			!if(LOUDER)print*,'Fugi done. K1,K2=',calcK1,calcK2
			if( ABS(calcK(1)-calcK(2)) < zeroTol)then
				if(LOUDER)write(dumpUnit,*) 'LLEFL: (K1-K2) ~ 0 in denom.'
				iErrCode=11
				exit
			endif
			...
			if(x1Lo < zeroTol .or. x1Lo > 1)then
				if(LOUDER)pause 'LLEFL: x1Lo out of range during K iteration.'
				iErrCode=17
				exit
			endif
			if(x1Up < zeroTol .or. x1Up > 1)then
				if(LOUDER)pause 'LLEFL: x1Up out of range during K iteration.'
				iErrCode=17
				exit
			endif
		...
		enddo !iter
		if(iter > itMax)iErrCode=16
		itMax=iter
		if(iErrCode > 9 .and. initialize==3)then
			iErrCode=3 !warning, answer is imprecise.
			x(1)=x1estLo
			x(2)=1-x(1)
			xU(1)=x1estHi
			xU(2)=1-xU(1)
			if(LOUDER)write(dumpUnit,*)'LLEFL: Warning: K-iteration failed. Using (imprecise) tangent line result.'
			if(LOUDER)write(dumpUnit,*)'LLEFL: x1estLo,x1estHi=',x1estLo,x1estHi
		endif
		zFeed(1:NC)=x(1:NC)/2+xU(1:NC)
		UOF=0.5
		bMixLo=sum( x(1:NC)*bVolCC_mol(1:NC))
		bMixUp=sum(xU(1:NC)*bVolCC_mol(1:NC))
		etaPasLo=rhoLo*bMixLo
		etaPasUp=rhoUp*bMixUp
		zPasLo=zLo
		zPasUp=zUp
	else ! ie. NC > 2.
		IF(Initialize==0)iErrCode=1	! nonzero on input signals need to compute initial guess.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!111
	SUBROUTINE LLSpinodals(NC,gamInf,x1SpinLo,GSpinLo,x1SpinHi,GSpinHi,x1minLo,GmixMinLo,x1minHi,GmixMinHi,x1con,d2G_dX1con,iErr) 
	!Compute the points where d2G/dX1^2=0 appear at given T.
	!Method: Golden Section search
	!Ref: NumRep 
	!Ref: 
	!AU: JRE, Aug 2010
	!	Input:
	!		LoHiX1 = 1 is searching for x1 starting from x1=0
	!Notes:
	!	This algorithm assumes 0.01 < x1spin < 0.99. If you have another situation, use x1=(1-K1)/(K2-K1).   
	USE GlobConst
	USE GibbsMin ! For passing tKelvin,pMPa, x1,d2G_dx1,dG_dx1,Gmix 
	IMPLICIT DoublePrecision(A-H,K,O-Z)
	External d2G_dX1sq,GmixMinusLine
	DoublePrecision xFrac(nmx),zFactor,d2G_dX1sq,GmixMinusLine
	DoublePrecision FUGC(nmx),gamInf(nmx) !,vMolec(nmx)
	LOGICAL LOUDER
	LOUDER=LOUD
	!LOUDER=.TRUE.
	!tKelvin USEd from GibbsMin
	!pMPa USEd from GibbsMin
	!GPure USEd from GibbsMin
...
611 format(1x,a,i8,8E12.4)
	...
	LIQ=1 !This is for LLE and we need aRes all the time.
	iErr=0
	tol=5.d-4
	xStep=1.d-4
	if(NC /= 2)then
		if(LOUDER)write(dumpUnit,*)'LLSpinodals: Only for NC=2. Sorry. T=',tKelvin
		iErr=11
		return
	endif
	nComps=NC
	xFrac(1:NC)=zeroTol  ! initialize.
	do iComp=1,NC
		xFrac(iComp)=1-zeroTol
		CALL FugiTP(tKelvin,PMPa,xFrac,NC,1,rhoMol_cc,zFactor,aRes,fugc,uRes,iErrF)
		GPure(iComp)=aRes+zFactor-1-LOG(zFactor)
		if(iComp==1)gamInf(2)=fugc(2)
		if(iComp==2)gamInf(1)=fugc(1)
		xFrac(iComp)=zeroTol
	enddo
	gamInf(1:NC)=exp( gamInf(1:NC)-GPure(1:NC) )
	x1Lo=1.2D-4
	x1Hi=1-x1Lo

	Call GoldenMin(d2G_dX1sq,x1Lo,x1Hi,tol,x1con,d2G_dX1con,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
	if(d2G_dX1con+zeroTol > 0)then
		iErr=12
		if(LOUDER)write(dumpUnit,611)'LLSpinodals: iErrMin, x1con, d2G_dX1^2 (> 0?),T ',iErrMin,x1con,d2G_dX1con,tKelvin 
		return
	endif
	if(LOUDER)write(dumpUnit,610)'LLSpinodals: Initial x1con,d2G_dX1con,GmixCon,dG_dX1con,T=',x1con,d2G_dX1con,Gmix,dG_dX1,tKelvin !Gmix USEd from GibbsMin
	slope=0			!USEd by GibbsMin for GmixMinusLine.
	constant=0
	if(gamInf(1) > 100)then
		x1minLo=1/gamInf(1)
		GmixMinLo=GmixMinusLine(x1minLo,iErrG)
	else	!look for Gmin in 0.01 < x1min < x1Con.
		Call GoldenMin(GmixMinusLine,x1Lo,x1con,tol,x1minLo,GmixMinLo,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
		if(iErrMin > 10)then
			iErr=13
			if(LOUDER)write(dumpUnit,611)'LLSpinodals: iErrMin, x1minLo, d2G_dX1^2 (> 0?),T ',iErrMin,x1minLo,d2G_dX1con,tKelvin 
			return
		endif
		if( ABS(x1minLo-x1con) < 2*tol )x1minLo=x1con/2	!It means there was no minimum on this interval other than x1con, so guesstimate the position of smallest slope.
	endif
	if(LOUDER)write(dumpUnit,610)'LLSpinodals: x1minLo,GmixMinLo,T=',x1minLo,GmixMinLo,tKelvin
	if(gamInf(2) > 100 )then
		x1minHi=1-1/gamInf(2)
		GmixMinHi=GmixMinusLine(x1minHi,iErrG)
	else !look for Gmin in x1con < x1min < 0.99. 
		Call GoldenMin(GmixMinusLine,x1con,x1Hi,tol,x1minHi,GmixMinHi,iErrMin)	!Properties like T,P,x,... must be passed by common or USEd.
		if(iErrMin > 10)then
			iErr=13
			if(LOUDER)write(dumpUnit,611)'LLSpinodals: iErrMin, x1minHi, d2G_dX1^2 (> 0?) ',iErrMin,x1minHi,d2G_dX1con 
			return
		endif
		if( ABS(x1minHi-x1con) < 2*tol )x1minHi=(1+x1con)/2	!It means there was no minimum on this interval other than x1con.
	endif ! gamInf > 100
	if(LOUDER)write(dumpUnit,610)'LLSpinodals: x1minHi,GmixMinHi,T=',x1minHi,GmixMinHi,tKelvin
	itMax=22
	! Search for lower spinodal (d2G/dX1^2 = 0 where x1 < x1con). 
	x1Old=x1con
	d2G_dX1old=d2G_dX1con
	x1New=x1Con-0.01D0
	d2G_dX1=1234
	change=1234
	iter=0
	do while( ABS(d2G_dX1) > tol .and. iter < itMax .and. ABS(change) > tol)
		iter=iter+1
		d2G_dX1=d2G_dX1sq(x1New,iErrD2G)
		if(iErrD2G > 0)then
			if(LOUDER)write(dumpUnit,*)'LLSpinodals: error from d2G_dX1sq during lower spinodal calculation. iErr=',iErrD2G
			exit
		endif
		if(	ABS(d2G_dX1-d2G_dX1old) < zeroTol)pause 'LLSpinodals: 0~(d2G_dX1-d2G_dX1old)' !=,(d2G_dX1-d2G_dX1old) 
		change=d2G_dX1/(d2G_dX1-d2G_dX1old)*(x1New-x1Old)
		x1Old=x1New
		d2G_dX1old=d2G_dX1
		if( (change) > x1New  )change=SIGN(x1New/2,change) 
		if( -change > 1-x1New )change=SIGN((1-x1New)/2,change)
		x1New=x1Old-change 
	enddo
	x1SpinLo=x1New
	if(iErrD2G > 0 .or. x1SpinLo < zeroTol)x1SpinLo=3*zeroTol
	GSpinLo=Gmix	!USEd from GibbsMin
	if(LOUDER)write(dumpUnit,610)'LLSpinodals: x1SpinLo,GmixSpinLo,T=',x1SpinLo,Gmix,tKelvin
	! Search for upper spinodal (d2G/dX1^2 = 0 where x1 > x1con). 
	x1Old=x1con
	d2G_dX1old=d2G_dX1con
	x1New=x1con+0.01d0
	d2G_dX1=1234
	...
	do while( ABS(d2G_dX1) > tol .and. iter < itMax .and. ABS(change) > tol)
		...
		d2G_dX1=d2G_dX1sq(x1New,iErrD2G)
		if(iErrD2G > 0)then
			if(LOUDER)write(dumpUnit,*)'LLSpinodals: error from d2G_dX1sq during upper spinodal calculation. iErr=',iErrD2G
			exit
		endif
		if(	ABS(d2G_dX1-d2G_dX1old) < zeroTol)pause 'LLSpinodals: 0~(d2G_dX1-d2G_dX1old)' !=,(d2G_dX1-d2G_dX1old) 
		change=d2G_dX1/(d2G_dX1-d2G_dX1old)*(x1New-x1Old)
	...
	enddo
	x1SpinHi=x1New
	if(iErrD2G > 0 .or. x1SpinHi > 1- zeroTol)x1SpinHi=1-3*zeroTol
	GSpinHi=Gmix	!USEd from GibbsMin
	if(LOUDER)write(dumpUnit,610)'LLSpinodals: x1SpinHi,GmixSpinHi,T=',x1SpinHi,Gmix,tKelvin
	return
	end


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!111
	SUBROUTINE LLMinDG_dX1(NC,tPas,pPas,LoHiX1,x1Spin,Gspin,dG_dx1Pas,iErr) 
	!Compute the points where d2G/dX1^2=0 appear at given T.
	!Method: Golden Section search
	!Ref: NumRep 
	!Ref: 
	...
	USE GibbsMin ! For passing tKelvin,pMPa, x1,d2G_dx1,dG_dx1,Gmix 
	IMPLICIT DoublePrecision(A-H,K,O-Z)
	DoublePrecision xFrac(nmx),zFactor
	DoublePrecision FUGC(nmx) !,gamInf(nmx) !,vMolec(nmx)
	LOGICAL LOUDER
	LOUDER=LOUD
	LOUDER=.TRUE.
...
611 format(1x,a,i8,8E12.4)
	...
	xStep=1.d-4
	if(NC /= 2)then
		write(dumpUnit,*)'LLSpinodal: Only for NC=2. Sorry.'
		iErr=11
		return
	endif
	nComps=NC
	x1=0.005D0
	if(LoHiX1==1)x1=0.99D0
	if(LOUD)write(dumpUnit,611)'LLSpinodal: T,x1init',tKelvin,x1
	xFrac(1)=x1+xStep/2
	xFrac(2)=1-xFrac(1)
	Call FugiTP( tKelvin,pMPa,xFrac,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrCode )
	if(zFactor < zeroTol)then
		iErr=11
		write(dumpUnit,*)'LLSpinodal: Z < 0 at x1Plus,Z=',xFrac(1),zFactor 
		return
	endif
	GmixPlus=aRes+zFactor-1-LOG(zFactor)-SUM(xFrac(1:2)*Gpure(1:2))	+ SUM( xFrac(1:2)*LOG(xFrac(1:2)) )
	xFrac(1)=x1-xStep/2
	xFrac(2)=1-xFrac(1)
	Call FugiTP( tKelvin,pMPa,xFrac,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrCode )
	if(zFactor < zeroTol)then
		iErr=11
		write(dumpUnit,*)'LLSpinodal: Z < 0 at x1minus,Z=',xFrac(1),zFactor 
		return
	endif
	GmixMinus=aRes+zFactor-1-LOG(zFactor)-SUM(xFrac(1:2)*Gpure(1:2)) + SUM( xFrac(1:2)*LOG(xFrac(1:2)) )
	dG_dx1Old=(GmixPlus-GmixMinus)/xStep
	dG_dx1=1234
	x1Old=x1
	change=1234
	itMax=55
	iter=0
	x1=x1Old+0.005d0
	do while(ABS(change) > tol .and. iter < itMax .and. ABS(dG_dx1) > tol)
		iter=iter+1
		xFrac(1)=x1+xStep/2
		xFrac(2)=1-xFrac(1)
		Call FugiTP( tKelvin,pMPa,xFrac,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrCode )
		if(zFactor < zeroTol .or.iErrCode>10)then
			iErr=11
			write(dumpUnit,611)'LLSpinodal: iErrCode, x1Plus, Z (< 0?) ',iErrCode,xFrac(1),zFactor 
			return
		endif
		GmixPlus=aRes+zFactor-1-LOG(zFactor)-SUM(xFrac(1:2)*Gpure(1:2)) + SUM( xFrac(1:2)*LOG(xFrac(1:2)) )
		xFrac(1)=x1-xStep/2
		...
		if(zFactor < zeroTol .or.iErrCode>10)then
			iErr=11
			write(dumpUnit,611)'LLSpinodal: iErrCode, x1minus, Z (< 0?) ',iErrCode,xFrac(1),zFactor 
			return
		endif
		GmixMinus=aRes+zFactor-1-LOG(zFactor)-SUM(xFrac(1:2)*Gpure(1:2)) + SUM( xFrac(1:2)*LOG(xFrac(1:2)) )
		dG_dx1=(GmixPlus-GmixMinus)/xStep
		if(ABS(dG_dx1-dG_dx1Old) < tol)then
			iErr=1
			write(dumpUnit,611)'LLSpinodal: Warning, dG/dx~dG/dxOld= ',iErrCode,dG_dx1,dG_dx1Old 
			return
		endif
		change=dG_dx1/(dG_dx1-dG_dx1Old)*(x1-x1Old)
		x1Old=x1
		dG_dx1Old=dG_dx1
		if(LOUDER)write(dumpUnit,610)'LLSpinodal: x1,dG/dx1,change=',x1,dG_dx1,change
		if(change > x1 )change=x1/2
		if( -change > 1-x1)change=(1-x1)/2 !otherwise we would find x1>1
		x1=x1-change
		if(x1 < zeroTol)x1=x1Old*( 1-SIGN(0.001d0,change) )
		if(x1 > (1-zeroTol) )x1=x1Old*( 1-SIGN(0.001d0,change) )
	enddo !while dev > tol
	Gspin=(GmixPlus+GmixMinus)/2
	x1Spin=x1
	dG_dX1Pas=dG_dX1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!111
	SUBROUTINE LLConsolute(NC,Tpas,Ppas,tFactor,itMax,x1Con,Tcon,Gcon,dG_dx1spin,uncert,iErr) 
...
611 format(1x,a,i8,8E12.4)
	...
	LIQ=1 !This is for LLE and we need aRes all the time.
	iErr=0
	tol=0.1 ! we don't need to be too precise. Tcon changes fast when the x's get anywhere close.
	xStep=1.d-4
	tKelvin=Tpas
	pMPa=Ppas
	if(NC /=2)then
		write(dumpUnit,*)'LLConsolute: Only for NC=2. Sorry.'
		iErr=11
		return
	endif
	nComps=NC
	...
	Tcon=tKelvin !initial guess
	TconOld=Tcon/tFactor
	!itMax=55
	diff=11
	iter=0
	do while( ABS(diff) > tol .and. iter < itMax .and. Tcon < Tmax .and. Tcon > Tmin) !step forward until diff<tol, then take one step back.
		iter=iter+1
		x1SpinHiOld=x1SpinHi
		x1SpinLoOld=x1SpinLo
		diffOld=diff
		GspinLoOld=GspinLo
		GspinHiOld=GspinHi  
		Tcon=TconOld*tFactor
		tKelvin=Tcon	! tKelvin, pMPa are USEd in GibbsMin.
		Call LLSpinodals(NC,gamInf,x1SpinLo,GSpinLo,x1SpinHi,GSpinHi,x1minLo,GmixMinLo,x1minHi,GmixMinHi,x1con,d2G_dX1con,iErrSpin) 
		if(iErrSpin > 9)then
			Tcon=TconOld
			write(dumpUnit,*)'LLConsolute: iErrSpin,Tcon=',iErrSpin,Tcon
			iErr=2
			return
		endif 
		diff=x1SpinHi-x1SpinLo
		if( ABS(diff) < tol .and. iter==1)then
			write(dumpUnit,*)'LLConsolute: x1SpinHi~x1SpinLo,Tcon=',x1SpinHi,x1SpinLo,Tcon
			iErr=11 ! Error: temperature appears to be outside of LL region.
			return
		endif
		TconOld=Tcon
		if(LOUDER)write(dumpUnit,611)'LLConsolute: iter,x1Lo,x1Hi,Tcon=',iter,x1SpinLo,x1SpinHi,Tcon
	enddo !while dif>tol
	if(iter==0)write(*,*)'LLConsolute: oops. Iter=0 after searching spinodals for Tcon?'
	uncert=(x1SpinHiOld-x1SpinLoOld)
	x1con =(x1SpinHiOld+x1SpinLoOld)/2
	TconOld=Tcon/tFactor
	dG_dx1spin=(GspinHiOld-GspinLoOld)/uncert
	Gcon     =(GspinHiOld+GspinLoOld)/2
	if(LOUDER)write(dumpUnit,611)'LLConsolute: x1 Done. iter,x1Lo,x1Hi,Tcon=',iter,x1SpinLoOld,x1SpinHiOld,TconOld
	if(TconOld < Tmin)then
		iErr=13
		write(dumpUnit,*)'LLConsolute: Tcon < Tmin'
		return
	endif
	if(TconOld > Tmax)then
		iErr=14
		write(dumpUnit,*)'LLConsolute: Tcon > Tmax'
		return
	endif
	if(iter > itMax-1)then
		iErr=15
		write(dumpUnit,*)'LLConsolute: iter > itMax'
		return
	endif

	!Found approximate x1con. Now refine Tcon such that d2G~0.
	xStep=uncert
	xFrac(1)=x1con
	xFrac(2)=1-xFrac(1)
	Call FugiTP( TconOld,pMPa,xFrac,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrCode )
	if(zFactor < zeroTol .or.iErrCode>10)then
		iErr=11
		write(dumpUnit,611)'LLConsolute: iErrCode, x1conInit, Z (< 0?) ',iErrCode,xFrac(1),zFactor 
		return
	endif
	GmixMid=aRes+zFactor-1-LOG(zFactor)-SUM(xFrac(1:2)*Gpure(1:2)) + SUM( xFrac(1:2)*LOG(xFrac(1:2)) )
	d2G_dx1Old=(GspinHiOld+GspinLoOld-2*GmixMid)/(xStep/2)**2
	Tcon=TconOld/1.02d0
	change=1234
	iter=0 
	do while( ABS(change) > tol .and. iter < 22 .and. Tcon < Tmax .and. Tcon > Tmin)
		iter=iter+1
		d2G_dx1=d2G_dX1sq(x1con,iErr)
		if(d2G_dx1 > 0)then
			Tcon=TconOld*1.001d0
			cycle
		endif
		change=d2G_dx1/(d2G_dx1-d2G_dx1Old)*(Tcon-TconOld)
		x1con=(x1SpinHi+x1SpinLo)/2
		if( ABS(change) > 1 )change=SIGN(1.d0,change) ! limit step size to 1K. 
		TconOld=Tcon
		d2G_dx1Old=d2G_dx1
		if(LOUDER)write(dumpUnit,611)'LLConsolute: iter,x1Lo,x1Hi,d2G,Tcon=',iter,x1SpinLo,x1SpinHi,d2G_dx1,Tcon
		Tcon=Tcon-change
	enddo 
	if(TconOld < Tmin)then
		iErr=13
		write(dumpUnit,*)'LLConsolute: After d2G iteration, Tcon < Tmin'
		return
	endif
	if(TconOld > Tmax)then
		iErr=14
		write(dumpUnit,*)'LLConsolute: After d2G iteration, Tcon > Tmax'
		return
	endif
	if(iter > 21)then
		iErr=1 !just a warning, because it must be better than Tcon from x1con search.
		write(dumpUnit,*)'LLConsolute: After d2G iteration, iter > itMax'
		return
	endif
	Gcon=GmixMid
	uncert=(x1SpinHi-x1SpinLo)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	DoublePrecision Function GmixMinusLine(x1,iErr)
	USE GlobConst
	USE GibbsMin !for tKelvin and PMPa, and to return dG_dX1, ...
	Implicit DoublePrecision(A-H,O-Z)
	DoublePrecision xFrac(nmx),FUGC(nmx),rLnGam(nmx)
	nComps=2 ! Sorry that's all that's available for now.
	LIQ=1
	xFrac(1)=x1
	xFrac(2)=1-x1
	...
	rLnGam(1:2)=FUGC(1:2)-Gpure(1:2)
	GEmix=SUM( xFrac(1:2)*rLnGam(1:2) )
	!d(nGE/RT)/dn1 = ln(gam1) = GE/RT+(d(GE/RT)/dx1)*(ndx1/dn1) = GE/RT+[d(GE/RT/dx1]*(1-x1)
	dG_dx1=(rLnGam(1)-GEmix)/(1-xFrac(1)) + LOG(xFrac(1)) -LOG(1-xFrac(1))
	Gis  = SUM( xFrac(1:2)*LOG(xFrac(1:2)) )
	!GEmix = aRes+zFactor-1-LOG(zFactor) - SUM(xFrac(1:2)*GPure(1:2)) - Gis
	Gmix  = GEmix+Gis
	GmixMinusLine=Gmix -( constant+slope*x1 ) 
	return
	end

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	DoublePrecision Function d2G_dX1sq(x1,iErr)
	USE GlobConst
	USE GibbsMin
	Implicit DoublePrecision(A-H,O-Z)
	DoublePrecision x(nmx),rLnGam(nmx),FUGC(nmx)
	nComps=2	! Sorry that's all that's available for now.
	LIQ=1		!Only for LLE.
	iErr=0
	d2G_dX1sq=0	!default if return with error.
	xStep=1.D-4
	if( x1-xStep < zeroTol .or. x1+xStep > 1-zeroTol)then
		iErr=11
		return
	endif
	x(1)=x1
	x(2)=1-x(1)
	Call FugiTP( tKelvin,pMPa,x,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrF )
	if(iErrF > 11)then
		iErr=11
		return
	endif
	GMix=aRes+zFactor-1-LOG(zFactor) - SUM(x(1:2)*GPure(1:2)) + x(1)*LOG(x(1))+x(2)*LOG(x(2))
	rLnGam(1:2)=FUGC(1:2)-Gpure(1:2)
	GEmix=SUM( x(1:2)*rLnGam(1:2) )
	!d(nGE/RT)/dn1 = ln(gam1) = GE/RT+(d(GE/RT)/dx1)*(ndx1/dn1) = GE/RT+[d(GE/RT/dx1]*(1-x1)
	dG_dx1=(rLnGam(1)-GEmix)/(1-x(1)) + LOG(x(1)) -LOG(1-x(1))
	x(1)=x1+xStep
	x(2)=1-x(1)
	Call FugiTP( tKelvin,pMPa,x,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrF )
	if(iErrF > 11)then
	...
	GPlus=aRes+zFactor-1-LOG(zFactor) - SUM( x(1:2)*GPure(1:2) ) + x(1)*LOG(x(1))+x(2)*LOG(x(2))
	rLnGam(1:2)=FUGC(1:2)-Gpure(1:2)
	GEPlus=SUM( x(1:2)*rLnGam(1:2) )
	!Gis=x1*ln(x1)+(1-x1)*ln(1-x1)
	!dGis_dX1=ln(x1)+1 -ln(1-x1)-1
	dG_dx1Plus=( rLnGam(1)-GEPlus )/(1-x(1))+LOG(x(1))-LOG(1-x(1))
	x(1)=x1-xStep
	x(2)=1-x(1)
	Call FugiTP( tKelvin,pMPa,x,nComps,LIQ,rhoMol_cc,zFactor,aRes,FUGC,uRes,iErrF )
	if(iErrF > 11)then
	...
	GMinus=aRes+zFactor-1-LOG(zFactor) - SUM(x(1:2)*GPure(1:2)) + x(1)*LOG(x(1))+x(2)*LOG(x(2))
	rLnGam(1:2)=FUGC(1:2)-Gpure(1:2)
	GEMinus=SUM( x(1:2)*rLnGam(1:2) )
	dG_dx1=(GPlus-GMinus)/(2*xStep)
	dG_dx1Minus=(rLnGam(1)-GEminus)/(1-x(1))+LOG(x(1))-LOG(1-x(1))
	d2G_dX1sq=(GPlus+GMinus-2*Gmix)/(xStep*xStep)
	d2G_dX1sq=(dG_dx1Plus-dG_dx1Minus)/(2*xStep)
	return
	end
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	SUBROUTINE GoldenMin(ExFun,parmLo,parmHi,tol,parmBest,funBest,iErr)	!Properties like T,P,x,... must be passed by common or USEd.
	!C
	...
	Implicit DoublePrecision(A-H,K,O-Z)
	External ExFun
	PARAMETER (RGOLD=.61803399d0,CGOLD=.38196602d0)
	LOGICAL LOUDER
	LOUDER=LOUD
	!LOUDER=.TRUE.
610	format(1x,a,12E12.4)
611	format(1x,a,i8,12E12.4)
	...
	parm0=parmLo
	parm3=parmHi
	parm1=parm0+CGOLD*(parm3-parm0)
	parm2=parm0+RGOLD*(parm3-parm0)
	fun0=ExFun(parm0,iErr)
	if(LOUDER)write(dumpUnit,610)' GoldenMin: parm0,fun0=',parm0,fun0
	fun1=ExFun(parm1,iErr)
	if(LOUDER)write(dumpUnit,610)' GoldenMin: parm1,fun1=',parm1,fun1
	fun2=ExFun(parm2,iErr)
	if(LOUDER)write(dumpUnit,610)' GoldenMin: parm2,fun2=',parm2,fun2
	fun3=ExFun(parm3,iErr)
	if(LOUDER)write(dumpUnit,610)' GoldenMin: parm3,fun3=',parm3,fun3
	TOL=0.001d0 ! 3 DECIMAL PLACES ON KIJ IS ENOUGH.
	iter=0      
	DO WHILE( ABS(parm3-parm0) > TOL ) !*(ABS(KIJ1)+ABS(KIJ2)))
		iter=iter+1
		if(iter > maxit)exit
		!IF(rms2 < rms1)THEN
		IF(fun2 < fun1)THEN
