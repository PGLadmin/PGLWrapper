...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	SUBROUTINE CritPure(NC,isZiter,toll,TC_Pure,VC_Pure,PC_Pure,ZC_Pure,Acen_pure,TbCalc,iErrCode)
	USE GlobConst
	USE BIPs
    IMPLICIT DOUBLEPRECISION(A-H,K,O-Z)
	PARAMETER(N=2)
...
! Specifying the step size for the iteration on T & V 
		stepT=1.d4
		stepV=1.d4
	IF (iEosOpt.eq.8.or.iEosOpt.eq.9) THEN
        stepT=10/toll
		stepV=10/toll
	ENDIF
	NC=1
...
! Initial guesses for TC and VC
	ZcPure=Zc(1)
	if(bESD)ZcPure=0.34
	VcExp=ZcPure*rGas*TC(1)/PC(1)
    xFrac(1)=1
    gmol(1)=1
	VC(1)=ZcPure*rGas*TC(1)/PC(1)
	tKelvin=TC(1)
	IF (iEosOpt==5 .or. iEosOpt==10) THEN
		VC(1)=bVolCC_mol(1)/0.11d0
		tKelvin=TC(1)*1.2 !add a littl to initial guess because TPT2 overestimates.
	ENDIF
	vTotCc=VC(1)
	etac=0.18
...
!usually the iteration converges after 10 cycles.
	...
	rms=SQRT(dP_dRho*dP_dRho+sqrt(d2P_dRho2*d2P_dRho2) )/1.414
	if(LOUDER)write(*,form610)' CritPure:T,eta,P,V,dP,d2P,rms=',tKelvin,eta,PMPa,vTotCc/1000,dP_dRho,d2P_dRho2,rms
	...
	!plotting shows that P vs. rho is simple but P vs. V goes concave up, then down, then up.
		...
		if(manual==1)then
			etaStor=etac
			errBest=1E11
			rhoBest=etaStor/bVolCC_mol(1)
			do iterD2P=1,12 !Iterate on rho to find d2P_dRho2=0
				eta=etaStor-0.007d0+iterD2P*0.001d0
				rho=eta/bVolCC_mol(1)
				vTotcc=1/rho
				rhoHi=(1+1/stepV)*rho
				rhoLo=(1-1/stepV)*rho
				call FuVtot(isZiter,tKelvin,1/rho,gmol,NC,FUGC,Zbase,aRes,uRes,iErrVtot)
		...
		do iterD2P=1,22 !Iterate on rho to find d2P_dRho2=0
            vTotcc=1/rho
            rhoHi=(1+1/stepV)*rho
            rhoLo=(1-1/stepV)*rho
	        call FuVtot(isZiter,tKelvin,1/rho,gmol,NC,FUGC,Zbase,aRes,uRes,iErrVtot)
			...
			stepLimit=0.1d0
			if(manual==1)stepLimit=0.005d0
			if (abs(Delta_Rho) > rho*stepLimit)Delta_Rho=Delta_Rho/(abs(Delta_Rho))*rho*stepLimit !Step limiting for V:
            rms=SQRT(dP_dRho*dP_dRho+sqrt(d2P_dRho2*d2P_dRho2) )/1.414
            if(rms < rmsBest)then
                tBest=tKelvin
                rhoBest=rho
			...
			rhoOld=rho
			rho=rho-Delta_Rho
			if(ABS(Delta_Rho/rho) < 5/stepV .and. ABS(d2P_dRho2) < 0.1)exit !exits do loop.
		...
		if (abs(Delta_T) > tKelvin*stepLimit) Delta_T=Delta_T/(abs(Delta_T))*tKelvin*stepLimit !Step limiting for T:
		dPold=dP_dRho
		Told=tKelvin
		...
		if(LOUDER)write(*,'(f9.4,3F7.4,3e12.4)')tKelvin,eta,PMPa,rhoG_cc,dP_dRho,d2P_dRho2,rms
		IF (iter > 22 .or. iErrCode.ne.0 .and. LOUD) THEN
			write(*,*) ' ' 
