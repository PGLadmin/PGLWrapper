...
The code can be used to build three sample projects described below in the section **Sample Projects (Details)**.

PGLEos.exe - A menu-driven command line tool to generate phase equilibria for a wide-variety of equations-of-state.

PGLDLL.dll - The .dll used to interface to the TDE engine for the analysis in Properties of Gases and Liquids, 6th Ed.

PGLDLLTest.exe - A command line tool to validate communication with PGLDLL.dll.

## Sample Uses of PGLEos.exe:
PGLEos.exe can generate phase diagrams, regress binary interaction parameters (BIPs), or reproduce the evaluations in PGL6ed. We provide a few examples below.
...
There are three projects that can be built easily using the codes from the PGLAdmin GitHub: PGLDLL, PGLDLLTest, and PGLEos. 

PGLDLL and its …Test function form the most expected use case. We presume that most readers will have their own flash codes and regression codes for optimizing BIPs and similar parameters. For them, we hope to facilitate incorporating the essential codes from PGL. We have had success calling PGLDLL from the C++ interface to TDE.

The plan for the TDEFree version is to include PGLDLL along with other thermo models.  A DLL might not be the easiest way to implement these codes, however. Readers may prefer to simply add these .f90 files into their existing projects. That should be easy, but don’t forget to carry along the PGLWrapper\input directory where the parameter files for the various EOSs are located.

For all the codes, you will need to configure the working directory for your implementation, typically by placing the final compiled executables in the root of the repository structure, and via adjusting the directory variables in the code.
...
The .sln and .vfproj files are examples and will require editing.

### PGLDLL

The PGLDLL was used for evaluations in Chapter 6 and Chapter 7 through the TDE interface. Those evaluation databases are not available. The project should be compiled as a shared .dll.

To build PGLDLL, add the following files:
...
To build the PGLDLLTest project, add the following files:

Folder SourceCodeUA:

    DLLTestMain

You will need to configure dependencies for compiling and linking. The main program needs to find the modules created from ModuleUAData when compiling and must link to PGLDLL.dll at runtime. This program should be compiled to link at runtime.

### PGLEos

Support for the PGLEos project has been added as an afterthought. We are happy to share these codes, but there are probably many superior codes available. 

The primary additions from PGLDLL to PGLEos are the MainMenu, MenuItems, FlashSubs, and MinPack. The MainMenu of PGLEos provides a crude console interface to a long list of computational options. Most of these are common bubble, dew, and flash routines for VLE, LLE, SLE, and SFE. A few options are available for critical points and parameter regression. 

We use the LmDif code from MinPack to perform most optimizations. The PGLEos project was used for evaluations of VLE, LLE, IDACs, and SLE in PGL6ed. The databases for these evaluations are included in the PGLWrapper\input directory.

Readers are welcome to use the PGLEos codes, but please do not expect support when, for example, a difficult flash in the critical region fails to converge. Readers are encouraged to develop their own codes for their own applications. We welcome learning about those developments. 

To build the PGLEos project, add the following files:
